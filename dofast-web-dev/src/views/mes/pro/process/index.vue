<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['pro:cess:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['pro:cess:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['pro:cess:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['pro:cess:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="processList" @selection-change="handleSelectionChange" ref="multipleTable" @row-click="handleRowClick">
      ...
      <el-table-column label="工序编码" align="center" prop="processCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['pro:cess:query']">{{ scope.row.processCode }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['pro:cess:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['pro:cess:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
      ...
      <el-divider content-position="center" v-if="form.id != null">操作步骤</el-divider>
      <Processcontent v-if="form.id != null" :optType="optType" :processId="form.id"></Processcontent>
...
export default {
  ...
  methods: {
    ...
    handleView(row) {
      this.reset();
      const processId = row.id || this.ids;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const processIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除生产工序？')
        .then(function () {
