<template>
  <div class="app-container">
    <el-row v-if="optType != 'view'" :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['pro:cess-content:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['pro:cess-content:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['pro:cess-content:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['pro:cess-content:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="processcontentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="100px" v-if="optType != 'view'" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['pro:cess-content:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['pro:cess-content:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    getList() {
      ...
      listProcesscontent(this.queryParams).then(response => {
        this.processcontentList = response.data.list;
    ...
    handleUpdate(row) {
      this.reset();
      const contentId = row.id || this.ids;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const contentIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除操作步骤？')
        .then(function () {
