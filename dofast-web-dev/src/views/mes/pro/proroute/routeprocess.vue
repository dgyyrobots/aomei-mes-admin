<template>
  <div class="app-container">
    <el-row :gutter="10" v-if="optType != 'view'" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['pro:route-process:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['pro:route-process:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['pro:route-process:delete']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="routeprocessList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" fixed="right" width="150px" v-if="optType != 'view'" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['pro:route-process:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['pro:route-process:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
            ...
            <el-form-item label="与下一道工序关系" prop="linkType">
              <el-tooltip effect="dark" placement="right">
                ...
                <el-select v-model="form.linkType" placeholder="请选择与下一道工序关系">
                  <el-option v-for="dict in dict.type.mes_link_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
                </el-select>
              </el-tooltip>
            </el-form-item>
          </el-col>
  ...
  methods: {
    ...
    getList() {
      ...
      listRouteprocess(this.queryParams).then(response => {
        this.routeprocessList = response.data.list;
