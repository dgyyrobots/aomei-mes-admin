  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      <el-form-item label="类型编码" prop="toolTypeCode">
        <el-input v-model="queryParams.toolTypeCode" placeholder="请输入类型编码" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="类型名称" prop="toolTypeName">
        <el-input v-model="queryParams.toolTypeName" placeholder="请输入类型名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="保养维护类型" prop="maintenType">
        <el-select v-model="queryParams.maintenType" placeholder="请选择保养维护类型" clearable>
          <el-option v-for="dict in dict.type.mes_mainten_type" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['tm:tool-type:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['tm:tool-type:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['tm:tool-type:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['tm:tool-type:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="tooltypeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型编码" align="center" prop="toolTypeCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['tm:tool-type:query']">{{ scope.row.toolTypeCode }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['tm:tool-type:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['tm:tool-type:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
          ...
          <el-col :span="8" v-if="form.codeFlag == 'Y'">
            <el-form-item label="保养维护类型" prop="maintenType">
              <el-select v-model="form.maintenType" placeholder="请选择保养维护类型">
                <el-option v-for="dict in dict.type.mes_mainten_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8" v-if="form.codeFlag == 'Y'">
...
<script>
import { listTooltype, getTooltype, getToolPage, delTooltype, addTooltype, updateTooltype } from '@/api/mes/tm/tooltype';
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        toolTypeCode: [{ required: true, message: '类型编码不能为空', trigger: 'blur' }],
        toolTypeName: [{ required: true, message: '类型名称不能为空', trigger: 'blur' }],
        codeFlag: [{ required: true, message: '是否启用不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listTooltype(this.queryParams).then(response => {
        this.tooltypeList = response.data.list;
    ...
    handleView(row) {
      this.reset();
      const toolTypeId = row.id || this.ids;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const toolTypeIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除工装夹具类型编号为"' + toolTypeIds + '"的数据项？')
        .then(function () {
