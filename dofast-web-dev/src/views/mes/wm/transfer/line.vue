<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-if="optType != 'view'" v-hasPermi="['wms:transfer-line:create']">新增 </el-button>
      </el-col>

      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-if="optType != 'view'" v-hasPermi="['wms:transfer-line:delete']">删除 </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="transferlineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" v-if="optType != 'view'" width="120px" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:transfer-line:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:transfer-line:delete']">删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="移入仓库" prop="toWarehouseId">
              <el-cascader v-model="warehouseInfo" :options="warehouseOptions" :props="warehouseProps" @change="handleToWarehouseChanged"></el-cascader>
            </el-form-item>
          </el-col>
          <el-col :span="8">
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        itemCode: [{ required: true, message: '请选择移出的物资', trigger: 'blur' }],
        toWarehouseId: [{ required: true, message: '请指定移入仓库', trigger: 'blur' }],
        quantityTransfer: [{ required: true, message: '转移数量不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listTransferline(this.queryParams).then(response => {
        this.transferlineList = response.data.list;
