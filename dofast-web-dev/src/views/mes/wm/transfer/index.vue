  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      <el-form-item label="转移单编号" prop="transferCode">
        <el-input v-model="queryParams.transferCode" placeholder="请输入转移单编号" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="转移单名称" prop="transferName">
        <el-input v-model="queryParams.transferName" placeholder="请输入转移单名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="转移单类型" prop="transferType">
        <el-select v-model="queryParams.transferType" placeholder="请选择转移单类型" clearable>
          <el-option v-for="dict in dict.type.mes_transfer_type" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="移出仓库" prop="fromWarehouseName">
        <el-input v-model="queryParams.fromWarehouseName" placeholder="请输入移出仓库名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="移入仓库" prop="toWarehouseName">
        <el-input v-model="queryParams.toWarehouseName" placeholder="请输入移入仓库名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:transfer:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['wms:transfer:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wms:transfer:delete']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="transferList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="转移单编号" align="center" prop="transferCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['wms:transfer:query']">{{ scope.row.transferCode }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-video-play" v-if="scope.row.status == 'PREPARE' && scope.row.transferType == 'INNER'" @click="handleExecute(scope.row)" v-hasPermi="['wms:transfer:update']">执行转移</el-button>
          <el-button size="mini" type="text" icon="el-icon-video-play" v-if="scope.row.status == 'PREPARE' && scope.row.transferType == 'OUTER'" @click="handleTransOut(scope.row)" v-hasPermi="['wms:transfer:update']">执行移出</el-button>
          <el-button size="mini" type="text" icon="el-icon-video-play" v-if="scope.row.status == 'CONFIRMED' && scope.row.transferType == 'OUTER'" @click="handleExecute(scope.row)" v-hasPermi="['wms:transfer:update']">执行移入</el-button>
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:transfer:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:transfer:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
          ...
          <el-col :span="8">
            <el-form-item label="转移单类型">
              <el-radio-group v-model="form.transferType">
                <el-radio v-for="dict in dict.type.mes_transfer_type" :key="dict.value" :label="dict.value">{{ dict.label }}</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="移出仓库" prop="fromWarehouseId">
              <el-select v-model="form.fromWarehouseId" placeholder="请选择移出仓库" clearable>
                <el-option v-for="dict in warehouseList" :key="dict.id" :label="dict.warehouseName" :value="dict.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
          ...
          <el-col :span="8">
            <el-form-item label="转移日期" prop="transferDate">
              <el-date-picker clearable v-model="form.transferDate" type="date" value-format="timestamp" placeholder="请选择转移日期"> </el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
      ...
      <el-card shadow="always" v-if="form.id != null" class="box-card">
        <Transferline ref="line" :transferId="form.id" :fromWarehouseId="form.fromWarehouseId" :toWarehouseId="form.toWarehouseId" :optType="optType"></Transferline>
      </el-card>
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        transferCode: [{ required: true, message: '转移单编号不能为空', trigger: 'blur' }],
        transferName: [{ required: true, message: '转移单名称不能为空', trigger: 'blur' }],
        transferType: [{ required: true, message: '转移单类型不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const transferId = row.id || this.ids;
      getTransfer(transferId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = '修改转移单';
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const transferIds = row.id || this.ids;
      this.$modal
