<template>
  <div class="app-container">
    <el-row v-if="optType != 'view'" :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:item-recpt-line:create']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wms:item-recpt-line:delete']">删除 </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="itemrecptlineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="100px" v-if="optType != 'view'" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:item-recpt-line:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:item-recpt-line:delete']">删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="有效期" prop="expireDate">
              <el-date-picker clearable v-model="form.expireDate" type="date" value-format="timestamp" placeholder="请选择有效期"> </el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
  ...
  methods: {
    ...
    getList() {
      ...
      listItemrecptline(this.queryParams).then(response => {
        this.itemrecptlineList = response.data.list;
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除物料入库单行编号为"' + lineIds + '"的数据项？')
        .then(function () {
