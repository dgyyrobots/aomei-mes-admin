<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:product-recpt-line:create']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-if="optType != 'view'" v-hasPermi="['wms:product-recpt-line:update']">修改 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-if="optType != 'view'" v-hasPermi="['wms:product-recpt-line:delete']">删除 </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="productrecptlineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-if="optType != 'view'" v-hasPermi="['wms:product-recpt-line:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-if="optType != 'view'" v-hasPermi="['wms:product-recpt-line:delete']">删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        quantityRecived: [{ required: true, message: '入库数量不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listProductrecptline(this.queryParams).then(response => {
        this.productrecptlineList = response.data.list;
