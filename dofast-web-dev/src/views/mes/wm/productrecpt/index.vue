<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:product-recpt:create']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['wms:product-recpt:update']">修改 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wms:product-recpt:delete']">删除 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['wms:product-recpt:export']">导出 </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="productrecptList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-delete" v-if="scope.row.status == 'PREPARE'" @click="handleExecute(scope.row)" v-hasPermi="['wms:product-recpt:update']">执行入库 </el-button>
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:product-recpt:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:product-recpt:delete']">删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="入库日期" prop="recptDate">
              <el-date-picker clearable v-model="form.recptDate" type="date" value-format="timestamp" placeholder="请选择入库日期"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
      ...
      <el-card shadow="always" v-if="form.id != null" class="box-card">
        <Productrecptline :recptId="form.id" :optType="optType" :workorderId="form.workorderId" :itemId="form.itemId"></Productrecptline>
      </el-card>
  ...
  methods: {
    ...
    getList() {
      ...
      listProductrecpt(this.queryParams).then(response => {
        this.productrecptList = response.data.list;
    ...
    handleDelete(row) {
      const recptIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除产品入库单编号为"' + recptIds + '"的数据项？')
        .then(function () {
