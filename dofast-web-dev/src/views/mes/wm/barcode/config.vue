<template>
  <el-dialog title="赋码设置" v-if="showFlag" :visible.sync="showFlag" :modal="false" width="80%" center>
    <el-table v-loading="loading" :data="barcodeconfigList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否自动生成" align="center" prop="autoGenFlag">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.autoGenFlag" active-value="Y" inactive-value="N" @change="handleAutoGenChange(scope.row)"></el-switch>
        </template>
      </el-table-column>
      <el-table-column label="默认打印模板" align="center" prop="defaultTemplate">
        <template slot-scope="scope">
          {{ scope.row.defaultTemplate }}
          <el-button type="text" @click="handleSetReport(scope.row)">设置</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listBarcodeconfig, getBarcodeconfig, addBarcodeconfig, updateBarcodeconfig } from '@/api/mes/wm/barcodeconfig';
  ...
  methods: {
    ...
    getList() {
      ...
      listBarcodeconfig(this.queryParams).then(response => {
        this.barcodeconfigList = response.data.list;
