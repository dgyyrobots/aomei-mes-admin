<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="条码类型" prop="barcodeType">
        <el-select v-model="queryParams.barcodeType" placeholder="请选择条码类型">
          <el-option v-for="dict in dict.type.mes_barcode_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
        </el-select>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:barcode:create']">新增</el-button>
      </el-col>

      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wms:barcode:delete']">删除</el-button>
      </el-col>

      <el-col :span="1.5">
        <el-button type="success" icon="el-icon-s-tools" size="mini" @click="handleConfig" v-hasPermi="['wms:barcode:update']">条码设置</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" icon="el-icon-printer" size="mini" @click="handleBatchPrint" v-hasPermi="['wms:barcode:update']">批量打印</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="barcodeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:barcode:update']">编辑</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:barcode:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
          ...
          <el-col :span="8">
            <el-form-item label="条码格式" prop="barcodeFormart">
              <el-select v-model="form.barcodeFormart" placeholder="请选择条码格式">
                <el-option v-for="dict in dict.type.mes_barcode_formart" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        barcodeFormart: [{ required: true, message: '条码格式不能为空', trigger: 'blur' }],
        barcodeType: [{ required: true, message: '条码类型不能为空', trigger: 'change' }],
        barcodeContent: [{ required: true, message: '产品物料ID不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const barcodeId = row.id || this.ids;
      getBarcode(barcodeId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = '修改条码';
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const barcodeIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除条码清单编号为"' + barcodeIds + '"的数据项？')
        .then(function () {
