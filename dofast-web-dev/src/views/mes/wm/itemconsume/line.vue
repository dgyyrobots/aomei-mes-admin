<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="消耗记录ID" prop="recordId">
        <el-input v-model="queryParams.recordId" placeholder="请输入消耗记录ID" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="消耗数量" prop="quantityConsume">
        <el-input v-model="queryParams.quantityConsume" placeholder="请输入消耗数量" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wm:itemconsumeline:add']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['wm:itemconsumeline:edit']">修改 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wm:itemconsumeline:remove']">删除 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['wm:itemconsumeline:export']">导出 </el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="itemconsumelineList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="行ID" align="center" prop="id" />
      <el-table-column label="消耗记录ID" align="center" prop="recordId" />
      ...
      <el-table-column label="消耗数量" align="center" prop="quantityConsume" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wm:itemconsumeline:edit']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wm:itemconsumeline:remove']">删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="消耗记录ID" prop="recordId">
          <el-input v-model="form.recordId" placeholder="请输入消耗记录ID" />
        </el-form-item>
        <el-form-item label="库存ID" prop="materialStockId">
        ...
        <el-form-item label="消耗数量" prop="quantityConsume">
          <el-input v-model="form.quantityConsume" placeholder="请输入消耗数量" />
        </el-form-item>
        <el-form-item label="领料批次号" prop="batchCode">
...
<script>
import { listItemconsumeline, getItemconsumeline, delItemconsumeline, addItemconsumeline, updateItemconsumeline } from '@/api/mes/wm/itemconsumeline';
  ...
  data() {
    return {
      ...
      rules: {
        ...
        quantityConsume: [{ required: true, message: '消耗数量不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listItemconsumeline(this.queryParams).then(response => {
        this.itemconsumelineList = response.data.list;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = '添加物料消耗记录行';
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const lineId = row.id || this.ids;
      getItemconsumeline(lineId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = '修改物料消耗记录行';
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateItemconsumeline(this.form).then(response => {
              this.$modal.msgSuccess('修改成功');
              this.open = false;
              this.getList();
            });
          } else {
            addItemconsumeline(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除物料消耗记录行编号为"' + lineIds + '"的数据项？')
        .then(function () {
          return delItemconsumeline(lineIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      this.download(
        'wm/itemconsumeline/export',
        {
          ...this.queryParams,
        },
        `itemconsumeline_${new Date().getTime()}.xlsx`,
      );
    },
  },
};
