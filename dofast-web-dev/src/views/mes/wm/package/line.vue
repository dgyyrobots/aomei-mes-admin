<template>
  <div class="app-container">
    <el-row :gutter="10" v-if="optType != 'view'" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:package-line:create']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wms:package-line:delete']">删除 </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="packagelineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" v-if="optType != 'view'" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:package-line:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:package-line:delete']">删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        packageId: [{ required: true, message: '装箱单ID不能为空', trigger: 'blur' }],
        itemId: [{ required: true, message: '产品物料ID不能为空', trigger: 'blur' }],
        quantityPackage: [{ required: true, message: '装箱数量不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listPackageline(this.queryParams).then(response => {
        this.packagelineList = response.data.list;
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除装箱明细编号为"' + lineIds + '"的数据项？')
        .then(function () {
