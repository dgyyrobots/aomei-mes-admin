  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      <el-form-item label="装箱单编号" prop="packageCode">
        <el-input v-model="queryParams.packageCode" placeholder="请输入装箱单编号" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="条码内容" prop="barcodeContent">
        <el-input v-model="queryParams.barcodeContent" placeholder="请输入条码内容" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="检查员名称" prop="inspectorName">
        <el-input v-model="queryParams.inspectorName" placeholder="请输入检查员名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:package:create']">新增</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="packageList" row-key="id" default-expand-all :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" @selection-change="handleSelectionChange">
      <el-table-column label="装箱单编号" width="180px" align="center" prop="packageCode">
        <template slot-scope="scope">
          <el-button type="text" size="mini" @click="handleView(scope.row)" v-hasPermi="['wms:package:query']">{{ scope.row.packageCode }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:package:update']" v-if="scope.row.status == 'PREPARE'">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:package:delete']" v-if="scope.row.status == 'PREPARE'">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="装箱日期" prop="packageDate">
                  <el-date-picker clearable v-model="form.packageDate" type="date" value-format="timestamp" placeholder="请选择装箱日期"> </el-date-picker>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
            ...
            <el-form-item label="箱长度" prop="packageLength">
              <el-input v-model="form.packageLength" placeholder="请输入箱长度" class="input-with-select">
                <el-select slot="append" class="width" v-model="form.sizeUnit">
                  <el-option v-for="item in measureOptions" :key="item.measureCode" :label="item.measureName" :value="item.measureCode"></el-option>
                </el-select>
              </el-input>
            </el-form-item>
          </el-col>
      ...
      <el-tabs type="border-card" v-if="form.id != null">
        <el-tab-pane label="子箱">
          <SubPackage ref="subPackge" :parentId="form.id" :optType="optType"></SubPackage>
        </el-tab-pane>
        <el-tab-pane label="装箱清单">
          <Packageline ref="packageLine" :packageId="form.id" :optType="optType"></Packageline>
        </el-tab-pane>
      </el-tabs>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="cancel" v-if="optType == 'view' || form.status != 'PREPARE'">返回</el-button>
        <el-button type="primary" @click="submitForm" v-if="form.status == 'PREPARE' && optType != 'view'">保 存</el-button>
        <el-button type="success" @click="doconfirm" v-if="form.status == 'PREPARE' && optType != 'view' && form.id != null">完成</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        parentId: [{ required: true, message: '父箱ID不能为空', trigger: 'blur' }],
        ancestors: [{ required: true, message: '所有父节点ID不能为空', trigger: 'blur' }],
        packageDate: [{ required: true, message: '装箱日期不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listPackage(this.queryParams).then(response => {
        this.packageList = this.handleTree(response.data.list, 'id', 'parentId');
    ...
    handleUpdate(row) {
      this.reset();
      const packageId = row.id || this.ids;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const packageIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除装箱单编号为"' + packageIds + '"的数据项？')
        .then(function () {
