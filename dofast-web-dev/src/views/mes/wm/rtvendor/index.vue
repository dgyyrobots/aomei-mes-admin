<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:rt-vendor:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['wms:rt-vendor:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wms:rt-vendor:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['mes:wm:rtvendor:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="rtvendorList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="退货单编号" align="center" prop="rtCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['wms:rt-vendor:query']">{{ scope.row.rtCode }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="200" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-delete" v-if="scope.row.status == 'PREPARE'" @click="handleExecute(scope.row)" v-hasPermi="['wms:rt-vendor:update']">执行退货</el-button>
          <el-button size="mini" type="text" icon="el-icon-edit" v-if="scope.row.status == 'PREPARE'" @click="handleUpdate(scope.row)" v-hasPermi="['wms:rt-vendor:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" v-if="scope.row.status == 'PREPARE'" @click="handleDelete(scope.row)" v-hasPermi="['wms:rt-vendor:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="退货日期" prop="rtDate">
              <el-date-picker clearable v-model="form.rtDate" type="date" value-format="timestamp" placeholder="选择时间" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
      ...
      <el-card shadow="always" v-if="form.id != null" class="box-card">
        <Rtvendorline ref="line" :rtId="form.id" :batchCode="form.batchCdoe" :vendorId="form.vendorId" :optType="optType"></Rtvendorline>
      </el-card>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        rtName: [{ required: true, message: '退货单名称不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      ...
      listRtvendor(this.queryParams).then(response => {
        this.rtvendorList = response.data.list;
