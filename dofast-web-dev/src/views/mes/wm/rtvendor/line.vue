    ...
    <el-table v-loading="loading" :data="rtvendorlineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="100px" v-if="optType != 'view'" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:rt-vendor:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:rt-vendor:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    getList() {
      ...
      listRtvendorline(this.queryParams).then(response => {
        this.rtvendorlineList = response.data.list;
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除供应商退货行编号为"' + lineIds + '"的数据项？')
        .then(function () {
