  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="生产任务ID" prop="taskId">
        <el-input v-model="queryParams.taskId" placeholder="请输入生产任务ID" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="生产任务名称" prop="taskName">
        <el-input v-model="queryParams.taskName" placeholder="请输入生产任务名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="生产日期" prop="produceDate">
        <el-date-picker clearable v-model="queryParams.produceDate" type="date" value-format="timestamp" placeholder="请选择生产日期"> </el-date-picker>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wm:productproduce:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['wm:productproduce:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wm:productproduce:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['wm:productproduce:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="productproduceList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="入库单ID" align="center" prop="recordId" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wm:productproduce:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wm:productproduce:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="生产日期" prop="produceDate">
          <el-date-picker clearable v-model="form.produceDate" type="date" value-format="timestamp" placeholder="请选择生产日期"> </el-date-picker>
        </el-form-item>
        <el-form-item label="单据状态">
          <el-radio-group v-model="form.status">
            <el-radio v-for="dict in dict.type.mes_order_status" :key="dict.value" :label="dict.value">{{ dict.label }}</el-radio>
          </el-radio-group>
        </el-form-item>
...
<script>
import { listProductproduce, getProductproduce, delProductproduce, addProductproduce, updateProductproduce } from '@/api/mes/wm/productproduce';
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listProductproduce(this.queryParams).then(response => {
        this.productproduceList = response.data.list;
    ...
    handleUpdate(row) {
      ...
      getProductproduce(recordId).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.recordId != null) {
            updateProductproduce(this.form).then(response => {
              this.$modal.msgSuccess('修改成功');
              this.open = false;
              this.getList();
            });
          } else {
            addProductproduce(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除产品产出记录编号为"' + recordIds + '"的数据项？')
        .then(function () {
          return delProductproduce(recordIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      this.download(
        'wm/productproduce/export',
        {
          ...this.queryParams,
        },
        `productproduce_${new Date().getTime()}.xlsx`,
      );
    },
  },
};
