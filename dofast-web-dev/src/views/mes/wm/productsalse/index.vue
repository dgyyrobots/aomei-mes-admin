  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="出库日期" prop="salseDate">
        <el-date-picker clearable v-model="queryParams.salseDate" type="date" value-format="timestamp"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['wms:product-salse:create']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['wms:product-salse:update']">修改 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['wms:product-salse:delete']">删除 </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="productsalseList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="出库单编号" width="150px" align="center" prop="salseCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['wms:product-salse:query']">
            {{ scope.row.salseCode }}
          </el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="120px" align="center" class-name="small-padding fixed-width" fixed="right">
        <template slot-scope="scope">
          <el-button size="mini" type="text" @click="handlePrint(scope.row)">打印</el-button>
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
            v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:product-salse:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
            v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:product-salse:delete']">删除 </el-button>
          <el-button size="mini" type="text" icon="el-icon-video-play" @click="handleExecute(scope.row)"
            v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:product-salse:update']">执行出库 </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="出库日期" prop="salseDate">
              <el-date-picker clearable v-model="form.salseDate" type="date" value-format="timestamp"
                placeholder="请选择出库日期"> </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="8">
      ...
      <el-tabs type="border-card" v-if="form.id != null">
        <el-tab-pane label="物料信息">
          <Productsalseline ref="line" :salseId="form.id" :warehouseId="form.warehouseId" :locationId="form.locationId"
            :areaId="form.areaId" :optType="optType" :saleNo="form.soCode"> </Productsalseline>
        </el-tab-pane>
        <el-tab-pane label="出库记录">
          <Productsalseline2 ref="line" :salseId="form.id" :warehouseId="form.warehouseId" :locationId="form.locationId"
            :areaId="form.areaId" :optType="optType" :saleNo="form.soCode"></Productsalseline2>
        </el-tab-pane>
      </el-tabs>
...
import Productsalseline2 from './line2.vue';
...
export default {
  ...
  components: { OqcSelectSingle, Productsalseline, ClientSelect, Productsalseline2 },
  data() {
    return {
      //自动生成编码
      autoGenFlag: false,
      ...
      rules: {
        ...
        clientCode: [{ required: true, message: '请指定客户', trigger: 'blur' }],
        salseDate: [{ required: true, message: '请选择出库日期', trigger: 'blur' }],
        soCode: [{ required: true, message: '销售订单编号不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listProductsalse(this.queryParams).then(response => {
        this.productsalseList = response.data.list;
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            ...
            addProductsalse(this.form).then(async response => {
              // 新增销售出库这里可以访问接口查询销售单编号下是否关联物料
              // confirm = await this.$modal.confirm('你需要导入销售单号关联物料吗?', '提示').catch(() => false);
    ...
    async handlePrint(row) {
      ...
      let datas = await createPrintLog({ printName: this.$store.state.user.nickname, printType: this.$route.meta.title + '-销售订单编号', printCode: row.soCode });
      if (!datas.data) {
        this.$message.error(datas.msg);
        return
      }
      let data = await getConfigKey('REPORT_SALES');
      window.open(`${process.env.VUE_APP_BASE_API}/jmreport/view/${data.data.value}?token=${getAccessToken()}&workorderId=${row.id}`);
    },
    /** 删除按钮操作 */
    handleDelete(row) {
      const salseIds = row.id || this.ids;
      this.$modal
