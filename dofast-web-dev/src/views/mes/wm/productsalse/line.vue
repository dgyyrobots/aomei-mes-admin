<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="productsalselineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="下单数量" align="center" prop="num" />
      ...
      <el-table-column label="操作" v-if="optType != 'view'" align="center" class-name="small-padding fixed-width" fixed="right">
        <template slot-scope="scope">
          <el-button type="text" icon="el-icon-plus" size="mini" @click="handleAdd(scope.row)" v-if="optType != 'view'" v-hasPermi="['wms:product-salse-line:create']">新增</el-button>
          <!-- <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
            v-if="optType != 'view' && !scope.row.del" v-hasPermi="['wms:product-salse-line:delete']">删除</el-button> -->
        </template>
      </el-table-column>
    </el-table>
...
import { mixinItems } from '@/api/mes/pro/workorder';
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        itemCode: [{ required: true, message: '出库物资不能为空', trigger: 'blur' }],
        warehouseInfo: [{ required: true, message: '仓库不能为空', trigger: 'blur' }],
        quantitySalse: [{ required: true, message: '出库数量不能为空', trigger: 'blur' }],
      },
      itemCode: null,
    };
  },
  ...
  methods: {
    ...
    async getList() {
      this.loading = true;
      let arr = [];
      let [{ data: a }, { data: b }] = await Promise.all([mixinItems(this.queryParams), listProductsalseline(this.queryParams)]);
      // console.log(a, b)
      if (a.total > b.total) {
        this.total = a.total;
      } else {
      ...
      let ars = a.list.map(item => {
        let as = b.list.filter(items => items.itemId == item.itemId);
        if (as.length) {
          arr = [...arr, ...as.splice(1, as.length)];
          return { num: item.num, ...as[0] };
        } else {
          return { del: true, ...item };
        }
      });
      let arb = b.list.filter(item => a.list.filter(items => items.itemId == item.itemId).length == 0);
      this.productsalselineList = [...ars, ...arr, ...arb];
      this.loading = false;

      // listProductsalseline(this.queryParams).then(response => {
      //   this.productsalselineList = response.data.list;
    ...
    handleSelectStock() {
      ...
      this.$refs.stockSelect.queryParams.itemCode = this.itemCode;
      this.$refs.stockSelect.itemFag = true;
