<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:sn:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['wms:sn:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="snList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="SN码数量" align="center" prop="snNum">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['wms:sn:query']">{{ scope.row.genDate }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:sn:update']">导出/打印</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:sn:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listSn, getSn, delSn, addSn } from '@/api/mes/wm/sn';
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        snNum: [{ required: true, message: '请输入需要生成的SN码数量', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      ...
      listSn(this.queryParams).then(response => {
        this.snList = response.data.list;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const snIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除SN码编号为"' + snIds + '"的数据项？')
        .then(function () {
          return delSn(snIds);
        })
        .then(() => {
