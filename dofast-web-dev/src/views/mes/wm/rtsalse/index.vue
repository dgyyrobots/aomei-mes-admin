<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['wms:rt-salse:create']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['wms:rt-salse:update']">修改 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['wms:rt-salse:delete']">删除 </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="rtsalseList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="退货单编号" width="140px" align="center" prop="rtCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['wms:rt-salse:query']">{{ scope.row.rtCode }} </el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="120px" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:rt-salse:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:rt-salse:delete']">删除 </el-button>
          <el-button size="mini" type="text" icon="el-icon-video-play" @click="handleExecute(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['wms:product-salse:update']">执行退货 </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="退货日期" prop="rtDate">
              <el-date-picker clearable v-model="form.rtDate" type="date" value-format="timestamp" placeholder="请选择退货日期"> </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="8">
      ...
      <el-divider v-if="form.id != null" content-position="center">产品信息</el-divider>
      <el-card shadow="always" v-if="form.id != null" class="box-card">
        <Rtsalseline :rtId="form.id" :optType="optType"></Rtsalseline>
      </el-card>
  ...
  methods: {
    ...
    getList() {
      ...
      listRtsalse(this.queryParams).then(response => {
        this.rtsalseList = response.data.list;
