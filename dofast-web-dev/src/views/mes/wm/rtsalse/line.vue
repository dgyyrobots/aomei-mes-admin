  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="rtsalselineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" v-if="optType != 'view'" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:rt-salse:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:rt-salse:delete']">删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        quantityRted: [{ required: true, message: '退货数量不能为空', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listRtsalseline(this.queryParams).then(response => {
        this.rtsalselineList = response.data.list;
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除产品销售退货行编号为"' + lineIds + '"的数据项？')
        .then(function () {
