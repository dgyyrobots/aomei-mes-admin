  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="物料料号" prop="itemCode">
        <el-input v-model="queryParams.itemCode" placeholder="请输入物料料号" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
...
<!--    <el-row :gutter="10" class="mb8">
    ...
    <el-table v-loading="loading" :data="ipqcList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="工序名称" width="120px" align="center" prop="processName" />
      <el-table-column label="工作中心" width="120px" align="center" prop="workstationName" />
      <el-table-column label="检测数量" width="120px" align="center" prop="quantityCheck" />
      <el-table-column label="不合格数" width="120px" align="center" prop="quantityUnqualified" />
      <el-table-column label="合格数" width="120px" align="center" prop="quantityQualified" />
      <el-table-column label="合格率" width="120px" align="center" prop="passRate">
        <template slot-scope="scope">
          {{ calculatePassRate(scope.row.quantityQualified, scope.row.quantityCheck) }}
        </template>
      </el-table-column>
...
<!--    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
          ...
          <el-col :span="8">
            <el-form-item label="检验类型" prop="ipqcType">
              <el-select v-model="form.ipqcType" placeholder="请选择检验类型">
                <el-option v-for="dict in dict.type.mes_ipqc_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="工单编码" prop="workorderCode">
              <el-input v-if="form.id == null" v-model="form.workorderCode" placeholder="请选择生产工单">
                <el-button slot="append" icon="el-icon-search" @click="handleWorkorderSelect"></el-button>
              </el-input>
              &lt;!&ndash;保存过则不允许修改工单，需要修改则删除重做&ndash;&gt;
              <el-input v-else v-model="form.workorderCode"></el-input>
            </el-form-item>
        ...
        <el-collapse>
          <el-collapse-item>
            <template slot="title"> 更多信息<i class="header-icon el-icon-info"></i></template>
            <el-row>
              <el-col :span="8">
      ...
      <el-card shadow="always" v-if="form.id != null" class="box-card">
        <Ipqcline ref="line" :ipqcId="form.id" :optType="optType"></Ipqcline>
      </el-card>
...
export default {
  ...
  methods: {
        ...
        if (valid) {
          if (this.form.id != null) {
            ...
            addIpqc(this.form).then(response => {
              ...
              this.form.id = response.data; //将返回的ID保存
              //this.open = false;
              this.getList();
            });
          }
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const ipqcIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除过程检验单编号为"' + ipqcIds + '"的数据项？')
        .then(function () {
    ...
    // 计算合格率
    calculatePassRate(qualified, quantityCheck) {
      if (quantityCheck === 0) return '0%'; // 防止除以零的情况
      const passRate = ((qualified / quantityCheck)) * 100;
      return `${passRate.toFixed(2)}%`; // 保留两位小数并加上百分号
    }
  },
};
</script>
