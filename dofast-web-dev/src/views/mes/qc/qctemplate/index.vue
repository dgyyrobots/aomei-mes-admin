  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      <el-form-item label="检测模板编号" prop="templateCode">
        <el-input v-model="queryParams.templateCode" placeholder="请输入检测模板编号" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="检测模板名称" prop="templateName">
        <el-input v-model="queryParams.templateName" placeholder="请输入检测模板名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="产品编码" prop="itemCode">
        <el-input v-model="queryParams.itemCode" placeholder="请输入产品名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['qms:template:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['qms:template:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['qms:template:delete']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="qctemplateList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="检测模板编号" align="center" prop="templateCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['qms:template:query']">{{ scope.row.templateCode }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['qms:template:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
      ...
      <el-tabs type="border-card" v-if="form.id != null">
        ...
        <el-tab-pane label="物料产品">
          <TemplateProduct ref="productTab" :templateId="form.id" :optType="optType"></TemplateProduct>
        </el-tab-pane>
      </el-tabs>
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        templateCode: [{ required: true, message: '检测模板编号不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      ...
      listQctemplate(this.queryParams).then(response => {
        this.qctemplateList = response.data.list;
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除检测模板编号为"' + templateIds + '"的数据项？')
        .then(function () {
