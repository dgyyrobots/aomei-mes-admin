<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['qms:defect:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['qms:defect:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['qms:defect:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['mes:qc:qcdefect:export']">导出</el-button>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const defectId = row.id || this.ids;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const defectIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除常见缺陷编号为"' + defectIds + '"的数据项？')
        .then(function () {
