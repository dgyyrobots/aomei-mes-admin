<template>
  <div class="app-container">
    <el-row v-if="optType != 'view'" :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['mes:md-product-sop:create']">新增</el-button>
      </el-col>
    </el-row>
    <div class="header">
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        itemId: [{ required: true, message: '物料产品ID不能为空', trigger: 'blur' }],
      },
      // 产品SOP表格数据
      sopList: [],
      //用于图片预览的清单
  ...
  methods: {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const sopIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除产品SOP编号为"' + sopIds + '"的数据项？')
        .then(function () {
