  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="单位编码" prop="measureCode">
        <el-input v-model="queryParams.measureCode" placeholder="请输入单位编码" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="单位名称" prop="measureName">
        <el-input v-model="queryParams.measureName" placeholder="请输入单位名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['mes:md-unit-measure:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['mes:md-unit-measure:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['mes:md-unit-measure:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['mes:md-unit-measure:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="unitmeasureList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:md-unit-measure:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:md-unit-measure:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="150px">
        ...
        <el-form-item label="主单位" v-if="form.primaryFlag == 'N'">
          <el-select v-model="form.primaryId" placeholder="请选择主单位">
            <el-option v-for="item in measureOptions" :key="item.id" :label="item.measureName" :value="item.id" :disabled="item.enableFlag == 'N'"></el-option>
          </el-select>
        </el-form-item>
  ...
  methods: {
    ...
    getList() {
      ...
      listUnitmeasure(this.queryParams).then(response => {
        this.unitmeasureList = response.data.list;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const measureIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除单位编号为"' + measureIds + '"的数据项？')
        .then(function () {
