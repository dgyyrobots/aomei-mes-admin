    ...
    <el-table v-loading="loading" :data="workstationmachineList">
      ...
      <el-table-column label="操作" v-if="optType != 'view'" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:md-workstation-machine:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
  ...
  methods: {
    ...
    getList() {
      ...
      listWorkstationmachine(this.queryParams).then(response => {
        this.workstationmachineList = response.data.list;
