    ...
    <el-table v-loading="loading" :data="workstationtoolList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" v-if="optType != 'view'" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:md-workstation-tool:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:md-workstation-tool:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="工装夹具类型" prop="toolTypeId">
          <el-select v-model="form.toolTypeId" placeholder="请选择类型">
            <el-option v-for="dict in toolTypeOptions" :key="dict.id" :label="dict.toolTypeName" :value="dict.id"></el-option>
          </el-select>
        </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        toolTypeId: [{ required: true, message: '工装夹具类型ID不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      ...
      listWorkstationtool(this.queryParams).then(response => {
        this.workstationtoolList = response.data.list;
