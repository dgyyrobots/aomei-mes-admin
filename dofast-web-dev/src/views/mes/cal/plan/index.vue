  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="开始日期" prop="startDate">
        <el-date-picker clearable v-model="queryParams.startDate" type="date" value-format="timestamp" placeholder="请选择开始日期"> </el-date-picker>
      </el-form-item>
      <el-form-item label="结束日期" prop="endDate">
        <el-date-picker clearable v-model="queryParams.endDate" type="date" value-format="timestamp" placeholder="请选择结束日期"> </el-date-picker>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['cal:plan:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['cal:plan:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['cal:plan:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="calplanList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="计划编号" align="center" prop="planCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['cal:plan:query']">{{ scope.row.planCode }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" v-if="scope.row.status == 'PREPARE'" @click="handleUpdate(scope.row)" v-hasPermi="['cal:plan:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" v-if="scope.row.status == 'PREPARE'" @click="handleDelete(scope.row)" v-hasPermi="['cal:plan:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-loading="formLoading" :visible.sync="open" width="960px" append-to-body>
          ...
          <el-col :span="12">
            <el-form-item label="轮班方式">
              <el-radio-group v-model="form.shiftType">
                <el-radio v-for="dict in dict.type.mes_shift_type" :key="dict.value" :label="dict.value">{{ dict.label }}</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col v-if="form.shiftType != 'SINGLE'" :span="6">
            <el-form-item label="倒班方式" prop="shiftMethod">
              <el-select style="width: 100px" v-model="form.shiftMethod" placeholder="请选择倒班方式">
                <el-option v-for="dict in dict.type.mes_shift_method" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col v-if="form.shiftMethod == 'DAY' && form.shiftType != 'SINGLE'" :span="6">
            <el-form-item label-width="20" prop="shiftCount">
              <el-input-number :min="1" controls-position="right" v-model="form.shiftCount"> </el-input-number>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
      ...
      <el-tabs type="border-card" v-if="form.id != null">
        <el-tab-pane label="班次">
          <Shift ref="shiftTab" :planId="form.id" :optType="optType"></Shift>
        </el-tab-pane>
        <el-tab-pane label="班组">
          <Team ref="teamTab" :planId="form.id" :calendarType="form.calendarType" :optType="optType"></Team>
        </el-tab-pane>
      </el-tabs>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        calendarType: [{ required: true, message: '请选择班组类型', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      ...
      listCalplan(this.queryParams).then(response => {
        this.calplanList = response.data.list;
