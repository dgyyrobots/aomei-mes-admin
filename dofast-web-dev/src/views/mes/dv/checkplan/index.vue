<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['cmms:dv-check-plan:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['cmms:dv-check-plan:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['cmms:dv-check-plan:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['mes:dv:checkplan:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="checkplanList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="计划编码" align="center" prop="planCode">
        <template slot-scope="scope">
          <el-button type="text" @click="handleView(scope.row)" v-hasPermi="['cmms:dv-check-plan:query']">{{ scope.row.planCode }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="130px" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['cmms:dv-check-plan:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-if="scope.row.status == 'PREPARE'" v-hasPermi="['cmms:dv-check-plan:delete']">删除</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleUpdate(scope.row)" v-if="scope.row.status == 'FINISHED'" v-hasPermi="['cmms:dv-check-plan:update']">停用</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
          ...
          <el-col :span="12">
            <el-form-item label="计划类型" prop="planType">
              <el-select v-model="form.planType" placeholder="请选择计划类型">
                <el-option v-for="dict in dict.type.dv_plan_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
          ...
          <el-col :span="4">
            <el-form-item prop="cycleType" label-width="80">
              <el-select v-model="form.cycleType" placeholder="请选择频率">
                <el-option v-for="dict in dict.type.mes_cycle_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="8">
            <el-form-item label="开始日期" prop="startDate">
              <el-date-picker clearable v-model="form.startDate" type="date" value-format="timestamp" placeholder="请选择开始日期"> </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="结束日期" prop="endDate">
              <el-date-picker clearable v-model="form.endDate" type="date" value-format="timestamp" placeholder="请选择结束日期"> </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="状态" prop="status">
      ...
      <el-tabs type="border-card" v-if="form.id != null">
        <el-tab-pane label="设备清单">
          <Checkmachinery ref="machinerylist" :optType="optType" :planId="form.id"></Checkmachinery>
        </el-tab-pane>
        <el-tab-pane :label="form.planType == 'CHECK' ? '点检项目' : '保养项目'">
          <Checksubject ref="subjectlist" :optType="optType" :planId="form.id"></Checksubject>
        </el-tab-pane>
      </el-tabs>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="cancel" v-if="optType == 'view' || form.status != 'PREPARE'">返回</el-button>
        <el-button type="primary" @click="submitForm" v-if="form.status == 'PREPARE' && optType != 'view'">保存</el-button>
        <el-button type="success" @click="handleFinish" v-if="form.status == 'PREPARE' && optType != 'view' && form.id != null">启用</el-button>
        <el-button type="success" @click="handleDeFinish" v-if="form.status == 'FINISHED' && optType != 'view' && form.id != null">停用</el-button>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        cycleType: [{ required: true, message: '请选择点检频率', trigger: 'blur' }],
        cycleCount: [{ required: true, message: '请输入点检次数', trigger: 'blur' }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listCheckplan(this.queryParams).then(response => {
        this.checkplanList = response.data.list;
    ...
    handleFinish() {
      if (this.form.id != null && this.form.status == 'PREPARE') {
        this.form.status = 'FINISHED';
        updateCheckplan(this.form).then(
          response => {
    ...
    handleDeFinish() {
      if (this.form.id != null && this.form.status == 'FINISHED') {
        this.form.status = 'PREPARE';
        updateCheckplan(this.form).then(
          response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const planIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除计划编号为"' + planIds + '"的数据项？')
        .then(function () {
