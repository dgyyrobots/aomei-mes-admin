<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" v-if="optType != 'view'" @click="handleAdd" v-hasPermi="['cmms:dv-check-machinery:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" v-if="optType != 'view'" :disabled="multiple" @click="handleDelete" v-hasPermi="['cmms:dv-check-machinery:delete']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="checkmachineryList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-delete" v-if="optType != 'view'" @click="handleDelete(scope.row)" v-hasPermi="['cmms:dv-check-machinery:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    getList() {
      ...
      listCheckmachinery(this.queryParams).then(response => {
        this.checkmachineryList = response.data.list;
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除点检设备编号为"' + recordIds + '"的数据项？')
        .then(function () {
