  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="类型名称" prop="machineryTypeName">
        <el-input v-model="queryParams.machineryTypeName" placeholder="请输入设备类型名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['cmms:dv-machinery-type:create']">新增</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="machinerytypeList" row-key="id" default-expand-all :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['cmms:dv-machinery-type:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-plus" @click="handleAdd(scope.row)" v-hasPermi="['cmms:dv-machinery-type:create']">新增</el-button>
          <el-button v-if="scope.row.parentTypeId != 0" size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['cmms:dv-machinery-type:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        machineryTypeName: [{ required: true, message: '设备类型名称不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      ...
      listMachinerytype(this.queryParams).then(response => {
        var Machinerytypes = this.handleTree(response.data, 'id', 'parentTypeId');
        this.machinerytypeList = Machinerytypes;
        this.loading = false;
      });
    },
    /** 转换设备类型数据结构 */
    ...
    handleUpdate(row) {
      ...
      getMachinerytype(row.id).then(response => {
    ...
    handleDelete(row) {
      this.$modal
        .confirm('是否确认删除设备类型编号为"' + row.id + '"的数据项？')
        .then(function () {
          return delMachinerytype(row.id);
        })
        .then(() => {
          this.getList();
