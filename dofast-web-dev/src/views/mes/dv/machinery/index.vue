...
export default {
  ...
  methods: {
    ...
    handleView(row) {
      ...
      this.getTreeselect();
      this.getWorkshops();
      const machineryId = row.id || this.ids;
      getMachinery(machineryId).then(response => {
        this.form = response.data;
        this.form.machineryTypeId = this.form.machineryTypeId + '';
        this.open = true;
        this.title = '查看设备信息';
        this.optType = 'view';
      });
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const machineryIds = row.id || this.ids;
      this.$modal
        .confirm('确认删除数据项？')
        .then(function () {
    ...
    /** 下载模板操作 */
    importTemplate() {
      this.download('mes/dv/machinery/importTemplate', {}, `md_item_${new Date().getTime()}.xlsx`);
    },
    // 文件上传中处理
