<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['cmms:dv-check-subject:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['cmms:dv-check-subject:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['cmms:dv-check-subject:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['mes:dv:dvsubject:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="dvsubjectList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['cmms:dv-check-subject:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['cmms:dv-check-subject:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
          ...
          <el-col :span="12">
            <el-form-item label="项目类型" prop="subjectType">
              <el-radio-group v-model="form.subjectType">
                <el-radio v-for="dict in dict.type.mes_dvsubject_type" :key="dict.value" :label="dict.value">{{ dict.label }}</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        subjectType: [{ required: true, message: '请选择项目类型', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      ...
      listDvsubject(this.queryParams).then(response => {
        this.dvsubjectList = response.data.list;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const subjectIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除设备点检保养项目编号为"' + subjectIds + '"的数据项？')
        .then(function () {
