<template>
  <div class="app-container">
    ...
    <el-card style="margin-top: 10px" v-loading="submitLoading" element-loading-text="提交支付中...">
      <!-- 支付宝 -->
      <el-descriptions title="选择支付宝支付"> </el-descriptions>
      <div class="pay-channel-container">
        <div class="box" v-for="channel in aliPayChannels" v-if="channelCodes.some(item => item == channel.code)"
          :key="channel.code" @click="submit(channel.code)">
      ...
      <div class="pay-channel-container">
        <div class="box" v-for="channel in wxPayChannels" :key="channel.code" @click="submit(channel.code)"
          v-if="channelCodes.some(item => item == channel.code)">
          <img :src="icons[channel.code]" />
          <div class="title">{{ channel.name }}</div>
        </div>
      </div>
...
import { getAppPage } from '@/api/pay/app'
export default {
  name: 'PayOrderSubmit',
  components: {
    QrcodeVue,
  ...
  created() {
    getAppPage({ pageSize: 100, merchantName: this.$store.state.user.name }).then(res => {
      // 根据商户id查询支付应用信息
      this.channelCodes = res.data.list[0].channelCodes
    })
