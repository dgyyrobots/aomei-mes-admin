<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="流程分类" prop="category">
        <el-select v-model="queryParams.category" placeholder="流程分类" clearable style="width: 240px">
          <el-option v-for="dict in categoryDictDatas" :key="parseInt(dict.value)" :label="dict.label"
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="操作" align="center" width="450" fixed="right">
        <template v-slot="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleCopy(scope.row)"
            v-hasPermi="['bpm:model:copy']">复制</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        ...
        <div v-if="form.id">
          ...
          <el-form-item v-if="form.formType === 20" label="表单提交路由" prop="formCustomCreatePath">
            ...
            <el-tooltip class="item" effect="light" content="自定义表单的提交路径，使用 Vue 的路由地址，例如说：bpm/oa/leave/create"
              placement="top">
              <i style="padding-left: 5px" class="el-icon-question" />
            </el-tooltip>
          </el-form-item>
    ...
    <copyModelDialog ref="copyModelDialog" />
  </div>
</template>

<script>
import { copyModel, deployModel, getModelPage, getModel, updateModelState, createModel, updateModel } from '@/api/bpm/model';
  ...
  methods: {
    ...
    handleCopy(row) {
      ...
      this.$modal
        .confirm('是否复制该流程！！')
        .then(function () {
          that.$refs['copyModelDialog'].initDialog(row);
        })
        .catch(() => { });
    },
    ...
    handleChangeState(row) {
      ...
      this.$modal
        .confirm('是否确认' + statusState + '流程名字为"' + row.name + '"的数据项?')
        .then(function () {
...
<style lang="scss">.my-process-designer {
