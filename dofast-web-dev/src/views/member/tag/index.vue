...
<script>
import { createTag, updateTag, deleteTag, getTag, getTagPage, exportTagExcel } from "@/api/member/tag";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除会员标签编号为"' + id + '"的数据项?').then(function() {
          return deleteTag(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有会员标签数据项?').then(() => {
          this.exportLoading = true;
          return exportTagExcel(params);
        }).then(response => {
          this.$download.excel(response, '会员标签.xls');
