            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                        v-hasPermi="['iot:device:update']">修改</el-button>
                    <el-button size="mini" type="text" icon="el-icon-caret-right" @click="handleView(scope.row)"
                        v-hasPermi="['iot:device:update']">定时详细</el-button><br />
                    ...
                    <el-button size="mini" type="text" icon="el-icon-caret-right" @click="handleRun(scope.row)"
                        v-hasPermi="['iot:device:update']">执行一次</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="120px">
                ...
                <el-form-item label="执行时间" prop="timerTimeValue">
                    <el-time-picker v-model="timerTimeValue" value-format="HH:mm" placeholder="选择时间" style="width:340px;"
                        @change="timeChange" :disabled="form.isAdvance == 1"></el-time-picker>
                </el-form-item>
                <el-form-item label="重复执行" prop="timerWeek">
                    <el-row>
                        <el-col :span="4">
                            <el-select v-model="timerRepeatValue" placeholder="请选择" @change="repeatChange"
                                :disabled="form.isAdvance == 1">
                                <el-option v-for="item in timerRepeats" :key="item.value" :label="item.label"
                                    :value="item.value">
                                </el-option>
                            </el-select>
                        </el-col>
                        <el-col :span="15" :offset="1" v-if="timerRepeatValue == 2">
                            <el-select v-model="timerWeekValue" placeholder="请选择" multiple style="width:485px"
                                @change="weekChange" :disabled="form.isAdvance == 1">
                    ...
                    <el-row v-for="(actionItem, index) in actionList" :key="index + 'action'" style="margin-bottom:10px;">
                        ...
                        <el-col :span="4" :offset="1">
                            <el-select v-model="actionItem.id" placeholder="请选择" v-if="actionItem.type == 1"
                                ...
                                <el-option v-for="(subItem, subIndex) in thingsModel.properties"
                                    :key="subIndex + 'property'" :label="subItem.name" :value="subItem.id">
                                </el-option>
                            </el-select>
                        ...
                        <el-col :span="10" :offset="1">
                            ...
                            <span
                                ...
                                <el-input style="width:180px;" v-model="actionItem.value" placeholder="值"
                                    :max="actionItem.thingsModelItem.datatype.max"
                                    :min="actionItem.thingsModelItem.datatype.min" type="number" />
                                <el-input style="width:70px;margin-left:15px;"
                                    v-model="actionItem.thingsModelItem.datatype.unit" placeholder="请输入整数或小数" disabled />
                            </span>
                            <span
                                v-else-if="actionItem.thingsModelItem && actionItem.thingsModelItem.datatype.type == 'bool'">
                                <el-switch v-model="actionItem.value"
                            ...
                            <span
                                ...
                                <el-select v-model="actionItem.value" placeholder="请选择" style="width:100%">
                                    <el-option v-for="(subItem, subIndex) in actionItem.thingsModelItem.datatype.enumList"
                                        :key="subIndex + 'things'" :label="subItem.text" :value="subItem.value">
                                    </el-option>
                                </el-select>
                            </span>
                            <span
