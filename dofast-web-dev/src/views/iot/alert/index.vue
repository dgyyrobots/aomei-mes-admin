  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="触发器" prop="triggers">
        <el-input v-model="queryParams.triggers" placeholder="请输入触发器" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="执行动作" prop="actions">
        <el-input v-model="queryParams.actions" placeholder="请输入执行动作" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="告警状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择告警状态（1-启动，2-停止）" clearable size="small">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['iot:alert:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['iot:alert:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['iot:alert:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['iot:alert:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="触发器" prop="triggers">
          <el-input v-model="form.triggers" placeholder="请输入触发器" />
        </el-form-item>
        <el-form-item label="执行动作" prop="actions">
          <el-input v-model="form.actions" placeholder="请输入执行动作" />
        </el-form-item>
        <el-form-item label="告警状态（1-启动，2-停止）" prop="status">
...
<script>
import { createAlert, updateAlert, deleteAlert, getAlert, getAlertPage, exportAlertExcel } from "@/api/iot/alert";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getAlertPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createAlert(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除设备告警编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有设备告警数据项?').then(() => {
          this.exportLoading = true;
          return exportAlertExcel(params);
        }).then(response => {
          this.$download.excel(response, '设备告警.xls');
