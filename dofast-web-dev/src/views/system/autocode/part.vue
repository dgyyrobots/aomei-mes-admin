  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="组成编码" prop="partCode">
        <el-input v-model="queryParams.partCode" placeholder="请输入组成编码" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="组成名称" prop="partName">
        <el-input v-model="queryParams.partName" placeholder="请输入组成名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['mes:auto-code-part:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['mes:auto-code-part:update']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['mes:auto-code-part:delete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="partList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:auto-code-part:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:auto-code-part:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
          ...
          <el-col :span="12">
            <el-form-item label="分段类型" prop="partType">
              <el-select v-model="form.partType">
                <el-option v-for="item in dict.type.sys_autocode_parttype" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="循环方式" prop="cycleMethod" v-if="form.partType == 'SERIALNO' && form.cycleFlag == 'Y'">
              <el-select v-model="form.cycleMethod">
                <el-option v-for="item in dict.type.sys_autocode_cyclemethod" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const partIds = row.id || this.ids;
      this.$modal
        .confirm('是否确认删除规则组成ID为"' + partIds + '"的数据项？')
        .then(function () {
