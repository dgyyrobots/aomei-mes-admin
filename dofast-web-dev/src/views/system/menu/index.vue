<template>
  <div class="app-container">
    ...
    <doc-alert title="菜单路由" url="https://doc.huizhizao.vip/vue2/route/" />
    <!-- 搜索工作栏 -->
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['system:menu:create']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-if="refreshTable" v-loading="loading" :data="menuList" row-key="id" :default-expand-all="isExpandAll" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" lazy :load="loadChildren">
      ...
      <el-table-column prop="status" label="状态" width="80">
        <template v-slot="scope">
          <dict-tag :options="menuStatusOptions" :value="scope.row.status" />
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:menu:update']">修改 </el-button>
          <el-button size="mini" type="text" icon="el-icon-plus" @click="handleAdd(scope.row)" v-hasPermi="['system:menu:create']">新增 </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['system:menu:delete']">删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="移动端图标" prop="mobileIcon">
              <ImageUpload v-model="form.mobileIcon" :value="form.mobileIcon" :limit="1" class="mall-image" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="菜单名称" prop="name">
          ...
          <el-col :span="12">
            <el-form-item label="菜单状态" prop="status">
              ...
              <el-radio-group v-model="form.status">
                <el-radio v-for="dict in this.getDictDatas(DICT_TYPE.COMMON_STATUS)" :key="dict.value" :label="parseInt(dict.value)">{{ dict.label }} </el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
const menuStatusOptions = [
  { value: '0', label: '开启', colorType: 'primary', cssClass: '' },
  { value: '1', label: '关闭', colorType: 'info', cssClass: '' },
];
export default {
  name: 'SystemMenu',
  components: { Treeselect, IconSelect, ImageUpload, ImagePreview },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      listMenu(this.queryParams).then(response => {
        const data = response.data.map(({ id, parentId, name, icon, sort, path, permission, component, componentName, status }) => {
          return {
            id,
            parentId,
            name,
        ...
        const menuTree = this.handleTree(data, 'id', 'parentId', 'children', 0, 'hasChildren');
        this.loadChildren = (tree, treeNode, resolve) => {
          this.$nextTick(() => {
            const nextMenuTree = this.handleTree(data, 'id', 'parentId', 'children', tree.id, 'hasChildren').map(item => {
              return {
                ...item,
                children: undefined,
              };
            });
            resolve(nextMenuTree);
          });
        };
        this.menuList = menuTree.map(item => {
          return {
            ...item,
            children: undefined,
          };
    ...
    getTreeselect() {
      listMenu().then(response => {
        this.menuOptions = [];
        const data = response.data.map(({ id, parentId, name }) => {
          return {
            id,
            parentId,
            name,
