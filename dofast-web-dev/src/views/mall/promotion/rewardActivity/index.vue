      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleClose(scope.row)"
            v-if="scope.row.status !== PromotionActivityStatusEnum.CLOSE.type && scope.row.status !== PromotionActivityStatusEnum.END.type"
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="活动时间" prop="startAndEndTime">
          <el-date-picker clearable v-model="form.startAndEndTime" type="datetimerange"
            :default-time="['00:00:00', '23:59:59']" value-format="timestamp" placeholder="选择开始时间" style="width: 480px" />
        </el-form-item>
        <el-form-item label="条件类型" prop="conditionType">
          <el-radio-group v-model="form.conditionType">
            <el-radio v-for="dict in this.getDictDatas(DICT_TYPE.PROMOTION_CONDITION_TYPE)" :key="dict.value"
        ...
        <el-form-item label="活动商品" prop="productScope">
          <el-radio-group v-model="form.productScope">
            <el-radio v-for="dict in this.getDictDatas(DICT_TYPE.PROMOTION_PRODUCT_SCOPE)" :key="dict.value"
              :value="dict.value" :label="parseInt(dict.value)">{{ dict.label }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="form.productScope === PromotionProductScopeEnum.SPU.scope" prop="productSpuIds">
          <el-select v-model="form.productSpuIds" placeholder="请选择活动商品" clearable size="small" multiple filterable
            style="width: 400px">
...
<script>
import { createRewardActivity, updateRewardActivity, deleteRewardActivity, getRewardActivity, getRewardActivityPage, closeRewardActivity } from '@/api/mall/promotion/rewardActivity';
import { PromotionConditionTypeEnum, PromotionProductScopeEnum, PromotionActivityStatusEnum } from '@/utils/constants';
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal
        .confirm('是否确认删除满减送活动编号为"' + id + '"的数据项?')
        .then(function () {
    ...
    handleClose(row) {
      const id = row.id;
      this.$modal
        .confirm('是否确认关闭满减送活动编号为"' + id + '"的数据项?')
        .then(function () {
