...
<template>
  <div class="container !flex-col !w-full !flex-items-stretch !bg-none">
      ...
      <el-tab-pane label="基础设置" name="basic">
        <el-form ref="basic" :model="baseForm" :rules="rules" label-width="100px" style="width: 95%">
          ...
          <el-form-item label="商品关键字" prop="keyword">
            <el-input v-model="baseForm.keyword" placeholder="请输入商品关键字" />
          </el-form-item>
          <el-form-item label="促销语">
            <el-input type="textarea" v-model="baseForm.sellPoint" placeholder="请输入促销语" />
          </el-form-item>
          <el-form-item label="商品简介" prop="introduction">
            <el-input v-model="baseForm.introduction" :rows="3" placeholder="请输入商品简介" type="textarea" />
          </el-form-item>
          ...
          <el-form-item label="单位" prop="unit">
            <el-select v-model="baseForm.unit" placeholder="请选择单位">
              <el-option v-for="dict in this.getDictDatas(DICT_TYPE.PRODUCT_UNIT)" :key="dict.value" :label="dict.label"
                :value="dict.value" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="运费模板" prop="deliveryTemplateId">
            <el-select v-model="baseForm.deliveryTemplateId" placeholder="请选择">
              <el-option v-for="item in deliveryTemplateList" :key="item.id" :label="item.name" :value="item.id" />
            </el-select>
          </el-form-item>
          <el-form-item label="商品分类" prop="categoryIds">
            <el-cascader v-model="baseForm.categoryIds" placeholder="商品分类" style="width: 100%" :options="categoryList"
              :props="propName" clearable />
          </el-form-item>
          ...
          <el-form-item label="分销类型" prop="subCommissionType">
            <el-radio-group v-model="baseForm.subCommissionType">
                ...
                <el-table-column fixed="right" label="操作" width="50" key="100">
                  <template v-slot="scope">
                    <el-button @click="scope.row.status = 1" type="text" size="small"
                      v-show="scope.row.status === undefined || scope.row.status === 0">禁用 </el-button>
                    <el-button @click="scope.row.status = 0" type="text" size="small" v-show="scope.row.status === 1"> 启用
                    </el-button>
                  </template>
                </el-table-column>
              </template>
      ...
      <el-tab-pane label="高级设置" name="senior">
        <el-form ref="senior" :model="baseForm" :rules="rules" label-width="100px" style="width: 95%">
          ...
          <el-form-item label="赠送积分" prop="giveIntegral">
            <el-input-number v-model="baseForm.giveIntegral" :min="0" />
          </el-form-item>
...
import { createSpu, getSpuDetail, updateSpu, getSimpleTemplateList } from '@/api/mall/product/spu';
...
export default {
  ...
  data() {
    return {
      ...
      checkboxGroup: [],
      recommendOptions: [{ name: '是否热卖', value: 'recommendHot' },
      ...
      rules: {
        ...
        picUrl: [{ required: true, message: '商品封面图地址不能为空', trigger: 'blur' }],
        deliveryTemplateId: [{ required: true, message: '运费模版不能为空', trigger: 'blur' }],
        unit: [{ required: true, message: '单位不能为空', trigger: 'blur' }],
        introduction: [{ required: true, message: '商品简介不能为空', trigger: 'blur' }],
        giveIntegral: [{ required: true, message: '赠送积分不能为空', trigger: 'blur' }],
      },
    };
  },
  async created() {
    ...
    this.deliveryTemplateList = (await getSimpleTemplateList()).data;
  ...
  methods: {
    onChangeGroup() {
      this.recommendOptions.forEach(({ value }) => {
        this.baseForm[value] = this.checkboxGroup.includes(value);
      })
    },
    removeSpec(index) {
    ...
    async submit() {
      try { await this.$refs['basic'].validate() } catch { return this.activeName = 'basic' }
      try { await this.$refs['rates'].validate() } catch { return this.activeName = 'rates' }
      try { await this.$refs['detail'].validate() } catch { return this.activeName = 'detail' }
      try { await this.$refs['senior'].validate() } catch { return this.activeName = 'senior' }
      this.$refs[this.activeName].validate(valid => {
        ...
        forms.recommendGood = Number(forms.recommendGood);
        forms.recommendNew = Number(forms.recommendNew);
        forms.recommendBest = Number(forms.recommendBest);
        forms.recommendHot = Number(forms.recommendHot);
        forms.recommendBenefit = Number(forms.recommendBenefit);
        forms.unit = Number(forms.unit);
        if (forms.id == null) {
          createSpu(forms)
            .then(() => {
    ...
    updateType(id) {
      getSpuDetail(id).then(response => {
        ...
        this.baseForm.keyword = data.keyword;
        this.baseForm.sellPoint = data.sellPoint;
        this.baseForm.picUrl = data.picUrl;
        this.baseForm.picUrls = data.picUrls;
        this.baseForm.introduction = data.introduction;
        this.baseForm.deliveryTemplateId = data.deliveryTemplateId;
        this.baseForm.unit = data.unit;
        if (this.ratesForm.spec === 2) {
          ...
          data.skus.forEach(r => {
            ...
            r.spec = [];
            (r.properties || []).map(p => {
              let obj = this.dynamicSpec.find(o => o.specId === p.propertyId);
              if (obj) {
                obj.specValue.push({
                  id: p.valueId,
                  name: p.valueName,
                ...
                obj.specName = p.propertyName;
                obj.specValue = obj.specValue || [];
                obj.specValue.push({
                  id: p.valueId,
                  name: p.valueName,
                });
