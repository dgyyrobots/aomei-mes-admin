 ...
 */
export function handleTree(data, id, parentId, children, rootId, hasChildren) {
  id = id || 'id';
  ...
  const treeData = cloneData.filter(father => {
    ...
    hasChildren ? (father[hasChildren] = branchArr.length > 0) : '';
...
// 过滤
export function treeFilter(children, filter, parent = null, deep = 0) {
  ...
  return [].concat(
    ...children.map(child => {
      ...
      if (child.children && child.children.length) {
        child.children = treeFilter(child.children, filter, child, deep + 1);
      }
      return typeof filter == 'function' ? filter(child, parent, deep) : child;
    }),
  );
}
