(function (win) {
  var CLODOP = {
    strWebPageID: "7BCAAAE", strTaskID: "", strHostURI: "http://localhost:18000",
    VERSION: "6.5.6.7", IVERSION: "6567", CVERSION: "6.5.6.7", HTTPS_STATUS: 0, VERSION_EXT: true,
    ...
    ADD_PRINT_PDF:
      function (top, left, width, height, strContent) {
        if (!this.blIslocal && strContent[1] == ":")
          alert("Unable to print or view remote service files"); else
    ...
    DoCPreview:
      function (destView, iWidth, iHigh, iOption) {
        var obody = document.body || document.getElementsByTagName("body")[0] || document.documentElement;
        if (typeof iWidth !== "number" || !iWidth) iWidth = Math.round(obody.offsetWidth * 2 / 3); else
        if (obody.offsetWidth < iWidth) iWidth = obody.offsetWidth;
        if (typeof iHigh !== "number" || !iHigh) iHigh = Math.round(window.screen.height - 200); else
    ...
    PopView:
      function (destView, strPURL, iWidth, iHigh) {
        try {
          if (destView && typeof destView === "string" && destView.length > 0) {
            if (destView === "_dialog") {
              if (window.showModalDialog) { window.showModalDialog(strPURL, 'dialog', 'center:yes'); } else { window.open(strPURL, "", "scrollbars=yes,toolbar=no,left=150,top=100,resizable=yes"); }
            } else
